...
def execute_order66(json_file_path, date_str, console, logger, credentials_file, tenant: str = typer.Option(None, "--tenant", "-t"), client_id: str = typer.Option(None, "--client-id", "-c"), client_secret: str = typer.Option(None, "--secret", "-s")):
        ...
        if Path(csv_file_path).exists() and Path(filtered_csv_file_path).exists() and Path(disabled_csv_file_path).exists():
            ...
            try:
                ...
                if excel_file_path.exists():
                    ...
                    FileManager.modify_excel_file(excel_file_path, logger)
                else:
